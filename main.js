...
;// CONCATENATED MODULE: ./src/js/domLogic.js
class DomLogic {
  constructor() {
  ...
  headerEvents() {
    const textareaOnBlurEventHandler = e => {
      if (e.type === "blur" || e.type === "keydown" && (e.key === "Enter" || e.key === "Escape")) {
        e.currentTarget.closest(".trello-list").querySelector("h2").textContent = e.currentTarget.value;
        e.currentTarget.classList.add("noshow");
        e.currentTarget.closest(".trello-list").querySelector("h2").classList.remove("noshow");
      }
    };
    ...
    Array.from(this.headers).forEach(header => header.addEventListener("click", headerOnEventHandler));
    const allTextAreas = document.querySelectorAll("textarea");
    Array.from(allTextAreas).forEach(textarea => textarea.addEventListener("input", autoResizeHandler));
  }
  addCardEvents() {
    const textareaOnBlurEventHandler = e => {
      if (e.type === "blur") {
        if (e.target.value.trim()) {
          this._cardCreator(e.target.closest(".list-cards").firstElementChild, e.target.value);
          e.target.value = "";
        }
        e.currentTarget.closest(".list-content").querySelector(".open-card-composer").classList.remove("noshow");
        e.currentTarget.closest(".list-card").classList.add("noshow");
        e.target.closest(".list-content").querySelector(".finish-add-card-btn").classList.add("noshow");
      }
    };
    ...
    const addCardLinkHandler = e => {
      ...
      const cardComposer = e.target.closest(".list-content").querySelector(".list-card");
      cardComposer.classList.remove("noshow");
      e.target.closest(".list-content").querySelector(".finish-add-card-btn").classList.remove("noshow");
    ...
    const addCardButtonEventHandler = e => {
      e.preventDefault();
      const textarea = e.target.closest(".list-content").querySelector(".list-card-composer-textarea");
      if (textarea.value.trim()) {
        this._cardCreator(textarea.closest(".list-cards").firstElementChild, textarea.value);
        textarea.value = "";
      }
    };
    Array.from(this.lists).forEach(list => {
      list.querySelector(".finish-add-card-btn .add-btn")?.addEventListener("mousedown", addCardButtonEventHandler);
    });
  }
  cardRemove() {
  ...
  cardDrag() {
    ...
    let mouseDownX = 0,
      mouseDownY = 0;
    Array.from(document.querySelectorAll(".cards-room")).forEach(cardsContainer => {
    ...
    const onMouseMoveHandler = e => {
      ...
      if (actualElement) {
        ...
        const cardsRoom = elementBelow?.closest(".list-content")?.querySelector(".cards-room");
        if (cardsRoom) {
  ...
  saveToLocalStorage() {
    window.addEventListener("beforeunload", () => {
      ...
      this.lists.forEach(list => {
        ...
        const listHeader = list.querySelector(".list-header-name-assist").textContent;
        if (listHeader.trim()) {
          kanbanData[`${listName}`] = {
            header: listHeader
          };
        ...
        if (cardsRoom.childElementCount > 0) {
          ...
          Array.from(cards).forEach(card => {
            listOfCardsNames.push(card.querySelector(".list-card-title").textContent);
          });
      ...
      if (kanbanData) {
        Object.keys(kanbanData).forEach(key => {
          ...
          if (kanbanData[key].header) {
            currentList.querySelector(".list-header-name-assist").textContent = kanbanData[key].header;
          }
